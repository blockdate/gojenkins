...
func (j *Job) InvokeCrumb(params map[string]string) (bool, error) {
	isQueued, err := j.IsQueued()
	if err != nil {
		return false, err
	}
	...
	var crumb Crumb
	respcrumb, errcrumb := j.Jenkins.Requester.GetJSON("/crumbIssuer/api/json",&crumb,nil)
	if errcrumb != nil {
		return false, err
	}
	if respcrumb.StatusCode != 200 && respcrumb.StatusCode != 201 {
		Error.Println("Could not get crumb %s", j.GetName())
		return false, errors.New("Could not get crumb " + j.GetName())
	}
	j.Jenkins.Requester.SetHeader(crumb.CrumbRequestField,crumb.Crumb)
	resp, err := j.Jenkins.Requester.Post(j.Base+endpoint, bytes.NewBufferString(data.Encode()), nil, nil)
	if err != nil {
		return false, err
	}
